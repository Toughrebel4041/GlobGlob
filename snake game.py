...
class Snake(Frame):
      def __init__(self):
            ...
            self.jigh_score_label.grid(row = 1, column = 2)

            self .new_game()
      def new_game(self):
            self.canvas.delete(ALL)
            self.canvas.create_text(WIDTH/2,HEIGHT/2-50, text="Welcome to the Game"
            + "\nclick window or press any arrow button"
            + "\nfor start this game..", tag="welcome_text")
            rectWidth = WIDTH/25
            rect1 = self.canvas.create_rectangle(WIDTH/2-rectWidth/2, HEIGHT/2-rectWidth/2, WIDTH/2+rectWidth/2\, HEIGHT/2+rectWidth/2, outline="#dbf", fill="#dbf"\, tag="rect1")rect2 = self.canvas.create_rectangle(WIDTH/2-rectWidth/2, HEIGHT/2-rectwidth/2, WIDTH/2+rectwidth/2\, HEIGHT/2+rectWidth/2, outline="#dbf", fill="#dbf"\, tag="rect2")
            rect3 = self.canvas.create_rectangle(WIDTH/2-rectWidth/2, HEIGHT/2-rectwidth/2, WIDTH/2+rectwidth/2\, HEIGHT/2+rectWidth/2, outline="#dbf", fill="#dbf"\, tag="rect3")
                                    ...
                                    if (os.path.isfile("high_score.txt")):
                                          ...
                                          self.high_score_label["text"] = "Highest Score: + str(self.high_score)
                                    ...
                                    def.create(self, event):
                                          ...
                                          if self.game_over == False:
                                                if event.keycode == 38: #for Linux the score is 111
                                                      self.direction = *up*
                                                elif event.keycode == 39: #for Linux the score is 114
                                                      self.direction = *right*
                                                elif event.keycode == 40: #for Linux the score is 116
                                                      self.direction = *down*
                                                elif event.keycode == 37: #for Linux the score is 113
                                                      self.direction = *left*
                                    ...
                                    def first_movement(self):
                                          ...
                                          if self,direction == "left":
                                                ...
                                                self.canvas.move("rect1",w,0)
                                                self.canvas.move("rect2",w,0)
                                          elif seld.direction == *up*:
                                                self.canvas.move("rect1",0,-w)
                                                self.canvas.move(100)
                                                self.canvas.move("rect1",0,-w)
                                                self.canvas.move("rect2",0,-w)
                                          self.canvas.after(100)
                                    def _move(self):
                                          w = self.rectWidth
                                          while True:
                                                sef.score_label["Text"] = "Score: " + str(self.score)
                                          ...
                                          int(frontCoodrs[0]-endCoords[0]),\
                                                           int(frontCoords[1]-endCoords[1])+w)
                                                                 elif self.direction == "right":

                                          self.canvas.move(self.canvas.gettags(endRect), int(frontCoords[0]-endCoords[0])+w,\
                                                           int(frontCoords[0]-endCoords[1]))
                                                                 elif self.directiion == "up"

                                          self.canvas.move(self.canvas.gettas(endRect), int(frontCoords[0]-endCoords[0]),\
                                                           int(frontCoords[1]-endCoords[1])-w)
                                                           self.canvas.after(100)
                                                           ...
                                                           self.check_collide()
                                                            ...
                                                            self.rectWidth*2) +self.rectWidth
                                                            ...
                                                            self.canvas.coords(self.rectangles[len(self.rectangles)-1])
                                                                       ...
                                                                       if self.direction == "left":
                                                                             ...
                                                                             x2 += w
                                                                              ...
                                                                              self.rectangles.append(thisReact)
                                                                              lock.release()
                                                                        ...
                                                                        def check_collide(self):
                                                                              frontCoords=self.canvas.find_overlapping(frontCoords[0],frontCoords[1]\,frontCoords[2],frontCoords[3])
                                                                              for item in overlapping:
                                                                                    if item == self.dot:
                                                                              #Snake collide with dot, grow snake and move dot
                                                                                          self.grow()
                                                                                          self.make_new_dot()
                                                                              ...
                                                                              if (self.lastDirection == "Left" and self.direction == "right") or\
                                                                                 ...
                                                                                 (self.lastDIrection == "Down" and self.direction == "Up") or\
                                                                                 self.end_game()
                                                                              def end_game(self):
                                                                                    self.game_over = True
